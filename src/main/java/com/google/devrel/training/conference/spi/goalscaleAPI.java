...
public class goalscaleAPI {
	...
	@ApiMethod(name = "saveClient", path = "/updateClient/{clientwebsafekey}/Update", httpMethod = HttpMethod.POST)
	public Client saveClient(@Named("clientwebsafekey") final String clientwebsafekey, ClientForm clientform)
		...
		int goalsettersize = goalsetterlist.size();
		for(int i =0 ; i < goalsettersize ; i++)
		{
			for(int j =0; j< goalsetterlist.get(i).getclients().size() ; j++)
			{
				if(goalsetterlist.get(i).getclients().get(j).getId().equals(client.getId()))
				{
					goalsetterlist.get(i).getclients().get(j).setId(clientform.getname()) ;
					goalsetterlist.get(i).getclients().get(j).setAge(clientform.getage());
					goalsetterlist.get(i).getclients().get(j).setLocation(clientform.getlocation());
					goalsetterlist.get(i).getclients().get(j).setDescription(clientform.getdescription());
					goalsetterlist.get(i).getclients().get(j).setname(clientform.getname());
					client.setId(clientform.getname()) ;
					client.setname(clientform.getname()) ;
					client.setAge(clientform.getage());
					client.setLocation(clientform.getlocation());
					client.setDescription(clientform.getdescription());
					gs = goalsetterlist.get(i);
					//gs.getclients().remove(j);
					//gs.getclients()
					break;
				}
			}						
		}
		ofy().save().entities(gs,client).now();
		return client;
	}
